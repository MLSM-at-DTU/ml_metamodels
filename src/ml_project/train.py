...
class TrainModel():
    ...
    def _save_data(self) -> None:
        ...
        for split in ["train", "val", "test"]:
            ...
            artifact = wandb.Artifact(
                name=f'{split}.pt',
                type='data',
                description=f"Processed {split} data.")
    ...
    def load_model(self) -> None:
        ...
        if self.cfg.model.layer_type == 'GCN':
            model = GCN(node_feature_dim=self.node_feature_dim, 
                        ...
                        num_gnn_layers = self.cfg.model.num_gnn_layers).to(self.cfg.train.device)
    ...
    def train(self) -> None:
        ...
        train_loader = DataLoader(train_data, batch_size=self.cfg.train.batch_size, shuffle=True, worker_init_fn=self._worker_init_fn)

        # Load validation dataset
        val_data = self.load_data(split = 'val')
        val_loader = DataLoader(val_data, batch_size=self.cfg.train.batch_size, shuffle=False, worker_init_fn=self._worker_init_fn)
        ...
        optimizer = torch.optim.AdamW(model.parameters(), lr=self.cfg.train.lr, weight_decay=self.cfg.train.weight_decay)
